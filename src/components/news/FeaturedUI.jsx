...
import { Search, ChevronLeft, ChevronRight, Clock, ArrowRight, Hash, TrendingUp, Zap, Monitor, Film, Microscope, FileText } from 'lucide-react';
    ...
    <Link href={`/news/${article.slug}`} className="group block h-full">
      <div className="w-[280px] h-[220px] bg-[#112240]/30 border border-[#e6f1ff]/5 rounded-xl overflow-hidden hover:border-[#64ffda]/40 transition-all duration-300 hover:shadow-lg hover:shadow-[#64ffda]/5 relative flex flex-col">
        <div className="h-32 overflow-hidden relative">
          ...
          <div className="absolute top-2 left-2 flex gap-1">
             {categories.slice(0, 1).map((cat, i) => (
                 <span key={i} className="text-[10px] font-bold bg-black/60 backdrop-blur text-[#64ffda] px-2 py-0.5 rounded border border-[#64ffda]/20 uppercase tracking-wider">{cat}</span>
             ))}
          </div>
        </div>
        <div className="p-3 flex flex-col flex-grow relative">
           ...
           <h3 className="relative text-sm font-bold text-[#e6f1ff] leading-snug line-clamp-2 group-hover:text-[#64ffda] transition-colors font-serif">{article.title}</h3>
           <div className="relative mt-auto flex items-center gap-2 text-[10px] text-[#8892b0] pt-2"><Clock size={10} /><span>{formatDate(article.createdAt || new Date())}</span></div>
        </div>
      </div>
    </Link>
  );
...
const CategoryTicker = ({ title, icon: Icon, articles = [], speed = "50s", reverse = false }) => {
  ...
  return (
    <div className="mb-12 relative">
        <div className="flex items-center gap-2 mb-4 px-4 sm:px-0"><div className="p-1.5 rounded bg-[#64ffda]/10"><Icon size={18} className="text-[#64ffda]" /></div><h2 className="text-lg font-bold text-[#e6f1ff] tracking-tight">{title}</h2><div className="h-[1px] flex-grow bg-gradient-to-r from-[#64ffda]/20 to-transparent ml-4"></div></div>
        <div className="group relative flex overflow-hidden -mx-4 sm:mx-0 sm:rounded-l-xl">
            ...
            <div className={`flex gap-4 animate-marquee hover:pause ${reverse ? 'animate-marquee-reverse' : ''}`} style={{ animationDuration: speed }}>
                {[...articles, ...articles, ...articles].slice(0, 15).map((article, i) => (
                    <div key={article._id + i} className="flex-shrink-0 transform transition-transform duration-300 hover:scale-105 hover:z-20"><MiniNewsCard article={article} /></div>
                ))}
            </div>
...
export default function FeaturedUI({ initialNews }) {
                        ...
                        {featuredStories.map((story, index) => (
                            <div key={story._id} className={`absolute inset-0 transition-all duration-700 ease-in-out ${index === currentSlide ? 'opacity-100 visible' : 'opacity-0 invisible'}`}>
                                <Link href={`/news/${story.slug}`} className="block w-full h-full relative cursor-pointer">
                                    ...
                                    <div className="absolute bottom-8 left-8 right-8 md:max-w-xl"><div className="backdrop-blur-md bg-[#0a192f]/60 border border-[#e6f1ff]/10 p-6 rounded-2xl shadow-xl animate-fade-in-up"><div className="flex gap-1 mb-2">{story.category.map((cat, i) => <span key={i} className="px-2 py-0.5 bg-[#64ffda] text-[#0a192f] text-[10px] font-extrabold uppercase rounded inline-block">{cat}</span>)}</div><h1 className="text-2xl md:text-3xl font-bold text-[#e6f1ff] mb-3 leading-tight font-serif hover:text-[#64ffda] transition-colors">{story.title}</h1><div className="text-[#64ffda] hover:text-[#e6f1ff] transition-colors flex items-center gap-1 text-sm font-bold">Read Now <ArrowRight size={14} /></div></div></div>
                                </Link>
                            </div>
                        ))}
                    ...
                    <div className="flex flex-col gap-4 h-full overflow-y-auto max-h-[500px] pr-2 custom-scrollbar">
                        {featuredStories.map((story, idx) => (
                            <Link href={`/news/${story.slug}`} key={story._id} onMouseEnter={() => setCurrentSlide(idx)} className={`group flex-1 min-h-[100px] transition-opacity ${currentSlide === idx ? 'opacity-100 ring-1 ring-[#64ffda]' : 'opacity-70 hover:opacity-100'}`}>
                                <div className="relative h-full w-full rounded-lg overflow-hidden border border-[#112240] shadow-lg bg-[#112240]/30 flex"><div className="w-1/3 h-full relative"><img src={story.imageUrl || "https://placehold.co/400x300"} className="absolute inset-0 w-full h-full object-cover opacity-80 group-hover:opacity-100" /></div><div className="w-2/3 p-4 flex flex-col justify-center bg-[#112240]/40"><h3 className="text-sm font-bold text-[#e6f1ff] leading-snug line-clamp-2 group-hover:text-[#64ffda]">{story.title}</h3></div></div>
                            </Link>
                        ))}
                    </div>
                </div>
            </div>
        ) : <div className="flex flex-col items-center justify-center py-20 opacity-50"><FileText size={64} className="text-[#64ffda] mb-4"/><h3 className="text-xl font-bold">No News Found</h3></div>}
